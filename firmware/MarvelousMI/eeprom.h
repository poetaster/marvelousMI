...
float u8ToFloat(uint8_t value) {
    return static_cast<float>(value / 255.00f);
}
...
void updateSlot() {
	...
	currentConfig.plaits[5] = floatToU8(plaits_engine);
	...
	currentConfig.rings[5] = floatToU8(rings_engine);
	...
	currentConfig.braids[5] = floatToU8(braids_engine );
	...
	currentConfig.clouds[5] = floatToU8(clouds_engine);
}
void updateValues(){

eeprom_error = u8ToFloat(currentConfig.plaits[0]);
/*
...
void loadMemorySlots() {
  ...
  //for (uint8_t slot = 0; slot < 8; slot++) {
    //int baseAddress = EEPROM_START_ADDRESS + (slot * CONFIG_SIZE);
    int baseAddress = EEPROM_START_ADDRESS + CONFIG_SIZE;
